...
def my_job():
    ...
    for subscription in subscribers_with_email:
        category_ids = subscriptions.filter(user=subscription).values_list('category', flat=True)  # <QuerySet [3, 2, 1, 4]>
        ...
        if new_posts.exists():
            ...
            logger.info(f"Sent email to {subscription.username} for new posts {post.title}")
